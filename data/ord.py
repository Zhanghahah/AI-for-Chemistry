...
_URL = "https://kgithub.com/open-reaction-database/ord-data/raw/main/data/"
_FILES = [
...
_HOMEPAGE = "https://kgithub.com/open-reaction-database/ord-data"
        ...
        features = datasets.Features(
            {
                ...
                "yield": datasets.Value("string"),
                "rxn_reference": datasets.Value("string"),
                "reaction_id": datasets.Value("string"),
    ...
    def _generate_examples(self, filepaths, split):
        ...
        for path in filepaths:
            ...
            for i in range(len(data.reactions)):
                    ...
                    for c in v.components:
                        if c.reaction_role == reaction_pb2.ReactionRole.REACTANT:
                            try:
                                ...
                                failed = True
                        elif c.reaction_role == reaction_pb2.ReactionRole.SOLVENT:
                            try:
                                solvents += f"{k}:" + next(
                                    i.value for i in c.identifiers
                                        if i.type == reaction_pb2.CompoundIdentifier.SMILES
                                )
                                solvents += ";"
                            except StopIteration:
                                pass
                                # solvents += ";"
                        elif c.reaction_role == reaction_pb2.ReactionRole.REAGENT:
                            try:
                                reagents += f"{k}:" + next(
                                    i.value
                                    for i in c.identifiers
                                ...
                                reagents += ";" # TODO: need to check if . is proper or not
                            except StopIteration:
                                pass

                        elif c.reaction_role == reaction_pb2.ReactionRole.CATALYST:
                            try:
                                catalysts += f"{k}:" + next(
                                    i.value
                                    for i in c.identifiers
                                ...
                                catalysts += ";" # TODO: need to check if . is proper or not
                            except StopIteration:
                                pass
                    ...
                    for p in outcome.products:
                        if p.reaction_role == reaction_pb2.ReactionRole.PRODUCT:
                            ...
                            try:
                                ...
                                rxn_yield += f"{out_index}:" + str(m.percentage.value)
                            except StopIteration:
                                rxn_yield += f"{out_index}:" + "None"
                            rxn_yield += ";"
                ...
                publication = rxn.provenance.publication_url
                yield key, {
                    "text": reactants + "||" + reagents + "||" + solvents + "||" +
                            catalysts + "||" + products,
                    "rxn_name": rxn_name,
                    "yield": rxn_yield,
                    "rxn_id": key,
                    "rxn_reference": publication,
